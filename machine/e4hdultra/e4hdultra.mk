...
flash-image-e4hdultra-disk:
	...
	zip -r $(IMAGE_DIR)/$(BOXTYPE)_$(shell date '+%d.%m.%Y-%H.%M')_recovery_emmc.zip disk.img imageversion lcdflashing.bmp
	# cleanup
...
flash-image-e4hdultra-rootfs:
	...
	#
	cd $(IMAGE_BUILD_DIR)/$(FLASHIMAGE_PREFIX) && \
	zip -r $(IMAGE_DIR)/$(BOXTYPE)_$(shell date '+%d.%m.%Y-%H.%M')_usb.zip rootfs.tar.bz2 kernel.bin imageversion lcdflashing.bmp
	# cleanup
	rm -rf $(IMAGE_BUILD_DIR)
