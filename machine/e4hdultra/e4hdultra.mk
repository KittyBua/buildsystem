...
$(D)/kernel: $(D)/bootstrap $(D)/kernel.do_compile
	install -m 644 $(KERNEL_DIR)/arch/arm/boot/$(KERNELNAME) $(TARGET_DIR)/boot/vmlinux
...
flash-image-e4hdultra-disk:
	...
	dd conv=notrunc if=$(TARGET_DIR)/boot/$(KERNELNAME) of=$(EMMC_IMAGE) bs=$(BLOCK_SIZE) seek=$(shell expr $(KERNEL_PARTITION_OFFSET) \* $(BLOCK_SECTOR))
...
flash-image-e4hdultra-online:
	...
	tar -cvzf $(IMAGE_DIR)/$(BOXTYPE)_$(shell date '+%d.%m.%Y-%H.%M')_online.tgz rootfs.tar.bz2 kernel.bin imageversion lcdflashing.bmp
	# cleanup
	rm -rf $(IMAGE_BUILD_DIR)
